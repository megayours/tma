...
const StickerPackAnimationContext = createContext<StickerPackAnimationContextType | undefined>(undefined);

interface StickerPackAnimationProviderProps {
  children: ReactNode;
}

export const StickerPackAnimationProvider: React.FC<StickerPackAnimationProviderProps> = ({ children }) => {
...
export const useStickerPackAnimationContext = (): StickerPackAnimationContextType => {
  const context = useContext(StickerPackAnimationContext);
  if (context === undefined) {
    throw new Error('useStickerPackAnimationContext must be used within a StickerPackAnimationProvider');
  }
  return context;
};